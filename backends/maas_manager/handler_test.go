...
func TestNewHandlerRejectsNonPostRequests(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/manager/api/login", nil)
...
func TestNewHandlerRejectsInvalidLoginRequest(t *testing.T) {
	...
	handler := NewHandler(config.AppConfig{}, url.URL{}, "allowed")
	rr := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/manager/api/login", strings.NewReader(`{"username":"alice"}`))
...
func TestNewHandlerReportsLDAPSearchFailure(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/manager/api/login", strings.NewReader(`{"username":"alice","password":"secret"}`))
	req.Header.Set("Content-Type", "application/json")

	handler(rr, req)

	if got.operation != "/manager/api/login" {
		t.Fatalf("operation = %q, want %q", got.operation, "/manager/api/login")
	}
	if got.logMessage != "LDAP search failed" {
		t.Fatalf("logMessage = %q, want %q", got.logMessage, "LDAP search failed")
	}
	if got.userMessage != "We could not verify your maas-manager access. Please try again or contact an administrator." {
